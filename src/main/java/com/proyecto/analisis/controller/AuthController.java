...
import com.proyecto.analisis.dto.EmailRequest;
import com.proyecto.analisis.dto.PasswordResetRequest;
import com.proyecto.analisis.dto.SecurityQuestionRequest;
...
public class AuthController {
   ...
   @GetMapping("/menu")
    ...
    public ResponseEntity<AuthResponse> changePassword(@RequestBody ChangePasswordRequest request, HttpServletRequest httpRequest) {
        String ip = httpRequest.getRemoteAddr();
        String userAgent = httpRequest.getHeader("User-Agent");
        String sistemaOperativo = UserAgentParser.getSistemaOperativo(userAgent);
        String dispositivo = UserAgentParser.getDispositivo(userAgent);
        String browser = UserAgentParser.getNavegador(userAgent);
        AuthResponse response = authService.changePassword(request, ip, userAgent, sistemaOperativo, dispositivo, browser);
        return ResponseEntity.ok(response);
    }
     @PostMapping("/reset-password")
    public ResponseEntity<AuthResponse> resetPassword(@RequestBody PasswordResetRequest request, HttpServletRequest httpRequest) {
        String ip = httpRequest.getRemoteAddr();
        String userAgent = httpRequest.getHeader("User-Agent");
        String sistemaOperativo = UserAgentParser.getSistemaOperativo(userAgent);
        String dispositivo = UserAgentParser.getDispositivo(userAgent);
        String browser = UserAgentParser.getNavegador(userAgent);
        AuthResponse response = authService.resetPassword(request, ip, userAgent, sistemaOperativo, dispositivo, browser);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/set-security-question")
    public ResponseEntity<AuthResponse> setSecurityQuestion(@RequestHeader("Authorization") String token,
                                                           @RequestBody SecurityQuestionRequest request) {
        AuthResponse response = authService.setSecurityQuestion(token, request.getPregunta(), request.getRespuesta());
        return ResponseEntity.ok(response);
    }

    @PostMapping("/security-question")
    public ResponseEntity<AuthResponse> getSecurityQuestion(@RequestBody EmailRequest request, HttpServletRequest httpRequest) {
        String ip = httpRequest.getRemoteAddr();
        String userAgent = httpRequest.getHeader("User-Agent");
        String sistemaOperativo = UserAgentParser.getSistemaOperativo(userAgent);
        String dispositivo = UserAgentParser.getDispositivo(userAgent);
        String browser = UserAgentParser.getNavegador(userAgent);
        AuthResponse response = authService.getSecurityQuestion(request.getCorreoElectronico(), ip, userAgent, sistemaOperativo, dispositivo, browser);
        return ResponseEntity.ok(response);
    }
}